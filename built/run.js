...
parser.add_argument("--debug-stacks", {
    help: "Attach the CDP Debugger and capture the JS call stack + local scope " +
...
parser.add_argument("--debug-native", {
    help: "With --debug-stacks, break on native functions (the document.cookie " +
...
parser.add_argument("--debug-encoding", {
    help: "With --debug-stacks --debug-native, also break on " +
...
parser.add_argument("--debug-breakpoint", {
    help: "With --debug-stacks, set an extra breakpoint at an arbitrary script " +
...
parser.add_argument("--probe", {
    help: "Pass-2 probe mode: pause at known script sites to capture the values a " +
...
parser.add_argument("--probe-targets", {
    help: "With --probe, a JSON file of breakpoint targets produced by " +
...
parser.add_argument("--debug-max-captures", {
    help: "With --debug-stacks, cap the total number of paused-stack captures so " +
...
parser.add_argument("--debug-max-value", {
    help: "With --debug-stacks, the max captured length of each variable value " +
...
parser.add_argument("--save-cookies", {
    help: "Dump the full cookie store (all first- and third-party cookies, including " +
...
parser.add_argument("--no-save-bodies", {
    help: "Do not record HTTP request/response bodies. Bodies are captured by default " +
...
parser.add_argument("--save-bodies-full", {
    help: "Keep every response body, not just textual ones. By default response " +
...
parser.add_argument("--body-max", {
    help: "Maximum bytes stored per body; longer bodies are truncated and marked. The " +
...
parser.add_argument("--bodies-budget-mb", {
    help: "Total megabytes of body content to store for the whole crawl. Once spent, " +
...
parser.add_argument("--recording-event-log", {
    help: "Write a durable GraphML event log to the output dir as PageGraph records " +
