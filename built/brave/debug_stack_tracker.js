...
export class DebugStackTracker {
    ...
    async enable(client) {
        ...
        if (this.#offsetBreakpoints.length > 0) {
            try {
                ...
                this.#log("enable", "armed beforeScriptExecution instrumentation pause");
            }
            catch (err) {
                this.#log("enable", `could not set the beforeScriptExecution pause (${String(err)}); ` +
    ...
    getUnarmedTargets() {
        ...
        const unmatchedInline = this.#inlineMisses.filter((m) => !this.#armedTargets.some((a) => a.spec === m.spec));
        return this.#offsetBreakpoints
            .filter((ob) => !seen.has(ob.spec))
            .filter((ob) => !unmatchedInline.some((m) => m.spec === ob.spec))
            .map((ob) => ({
            skipped: "script never parsed — no script matching this URL pattern loaded. " +
    ...
    registerTargets(targets) {
        for (const tgt of targets) {
            try {
                ...
                this.#log("registerTargets", `bad target ${tgt.urlRegex}: ${String(err)}`);
            }
        }
        this.#log("registerTargets", `registered ${String(targets.length)} probe target(s)`);
    }
            ...
            try {
                ...
                if (ob.expectedSha256 !== undefined) {
                    if (actual !== ob.expectedSha256) {
                        ...
                        this.#log("armOffsetsForScript", `SKIPPED ${ob.spec}: ${event.url} hash ${actual.slice(0, 12)} != ` +
                ...
                if (ob.requirePrecedingSource !== undefined) {
                    ...
                    if (!preceding.endsWith(ob.requirePrecedingSource)) {
                        ...
                        this.#inlineMisses.push({
                            skipped: `inline script did not match the target's context (expected ` +
                ...
                const columnNumber = inSource.lineNumber === 0
                ...
                this.#armedTargets.push({
                    ...
                    hashMatchesPass1Response: ob.pass1ResponseSha256 === undefined
                        ? undefined
