...
const createPartialGraphMLPath = (args, url) => {
...
export const findOrphanGraphML = async (outDir, logger) => {
    try {
...
export const findEventLog = async (outDir, logger) => {
    try {
        const entries = await readdir(outDir, { withFileTypes: true });
        let newest = null;
        for (const entry of entries) {
...
export const recoverPartialGraphML = async (args, url, headersLogger, logger) => {
