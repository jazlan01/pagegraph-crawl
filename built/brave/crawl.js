...
const generatePageGraph = async (client, logger) => {
...
const dumpCookieStore = async (client) => {
    try {
        ...
        try {
            ...
            if (args.debugStacks) {
                ...
                if (args.probeTargets !== undefined) {
                    try {
                        const parsed = JSON.parse(readFileSync(args.probeTargets, "utf8"));
                        const list = Array.isArray(parsed)
                            ? parsed
                            : (parsed.targets ?? []);
                        const usable = list.filter((tgt) => typeof tgt.urlRegex === "string" &&
                            typeof tgt.offset === "number");
                        stackTracker.registerTargets(usable);
                        logger.info("probe: loaded ", String(usable.length), " target(s) from ", args.probeTargets);
                    }
                    catch (err) {
                        logger.error("probe: could not read --probe-targets: ", String(err));
                    }
                }
            ...
            const metadataTracker = new RequestMetadataTracker(logger, false, bodyLog);
            ...
            if (args.saveCookies) {
                await writeCookies(args, urlToCrawl, JSON.stringify(await dumpCookieStore(client), null, 2), logger);
                ...
                await writeCookieNetwork(args, urlToCrawl, metadataTracker.toCookieNetworkJSON(), logger);
                ...
                await writeRedirects(args, urlToCrawl, metadataTracker.toRedirectChainsJSON(), logger);
            }
            if (args.debugStacks && stackTracker) {
                ...
                const payload = args.probe
                    ? {
                        ...
                        caveat: "captured on a separate page load from the pass-1 graph; " +
                ...
                if (unarmed.length > 0) {
                    logger.error("probe: ", String(unarmed.length), " target(s) never parsed — the matching script did not load. If " +
                        "this browser blocks trackers, it blocked the probe target.");
                }
                if (skipped.length > 0) {
                    logger.info("probe: ", String(skipped.length), " target(s) skipped (script changed since pass 1)");
                }
                await writeStacks(args, urlToCrawl, JSON.stringify(payload, null, 2), logger);
            }
            ...
            if (bodyLog !== undefined) {
                ...
                logger.info("Recorded bodies: ", `${String(stats.bodiesStored)} of ${String(stats.records)} records, ` +
