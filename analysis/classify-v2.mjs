...
Transmissions and \`bodyExfil.perDest\` may also carry \`sentForm\` and \`carriesIdentifier\`, derived deterministically from the actual outbound bytes. \`sentForm\`: "raw" — the stored value left as-is; "re-encoded" — mechanically re-encoded (base64/urlencoded), still the same value; "fragment" — a piece of the value left without the rest; "derived" — JS computed something from the value before sending. \`carriesIdentifier\`: true — the outbound bytes included a stable identifier-grade token (a UUID, a long digest, a high-entropy id); false — only non-identifying parts (flags, timestamps, recorded choices) left; "unknown" — not determinable. These change what a transmission proves: a js-initiated send whose outbound bytes carry NO identifier is weak tracking evidence — it may be state propagation (settings, consent choices) rather than identification of the visitor — while carriesIdentifier=true to a third party is the strong form of the same evidence. Weigh destination, channel and carriesIdentifier together.
...
const classifyOne = async (name) => {
  ...
  return {
    ...
    features: f,
    // Per-destination outbound records WITH the actual request bytes (excerpt + highlight
    // offsets). Lives here — not in `features` (which stays value-free for Pass B) — so Pass C
    // and the report can show WHAT left to each destination.
